 ...
 *
 * Dieselbe Rechnung steht als JavaScript in `scripts/lib/laea.mjs` — dort baut
 * sie die Pakete, hier liest sie sie. Beide Fassungen prüft
 * `scripts/check-population.mts` gegeneinander und gegen amtliche Stützpunkte
 * (gemessen: 0,5 mm Abweichung gegen pyproj).
 */
...
function qOf(sinPhi: number): number {
  const es = E * sinPhi;
  return (
    (1 - E2) *
...
const betaOf = (q: number): number => Math.asin(q / Q_P);
...
export function toLaea(lat: number, lon: number): [number, number] {
  const phi = lat * RAD;
...
export function fromLaea(x: number, y: number): [number, number] {
